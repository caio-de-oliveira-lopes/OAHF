...
class AlwabpSolution(Solution):
    ...
    def __init__(
        ...
        self._reverse_default_graph_orientation = GraphOrientation.BACKWARD
        self._best_worker_for_task: Dict[int, int] = {task: 0 for task in self.tasks}
        self._workers_ranks: Dict[int, Dict[int, int]] = {
            task: {worker: 0 for worker in self.workers} for task in self.tasks
        }
    ...
    @property
    def reverse_default_graph_orientation(self) -> GraphOrientation:
        """
        Retrieves the reverse default graph orientation of the solution.

        Returns:
            GraphOrientation: The reverse of the current default graph orientation.
        """
        return self._reverse_default_graph_orientation
    ...
    def default_graph_orientation(self, graph_orientation: GraphOrientation) -> None:
        ...
        old_hash = hash(self)
        self._reverse_default_graph_orientation = self._default_graph_orientation
    ...
    def add_worker_to_station(
        ...
        try:
            if self.worker_station_assignment[worker] is None:
                ...
                LogManager.invalid_action(
                    "add worker to station, it was already assigned to one", self.name
                )
                return False
        except Exception as e:
    ...
    def remove_worker_from_station(
        ...
        try:
            if self.station_worker_assignment.get(station) == worker:
                ...
                LogManager.invalid_action(
                    "remove worker from station, it wasnt assigned to it", self.name
                )
                return False
        except Exception as e:
    ...
    def add_task_to_station(self, task: int, station: int) -> bool:
        ...
        try:
            if self.task_station_assignment[task] is None:
                ...
                self._station_cycle_time_memo[station] += self.get_task_execution_time(
                    task, worker
                )
                ...
                LogManager.invalid_action(
                    "add task to station, it was already assigned to one", self.name
                )
                return False
        except Exception as e:
    ...
    def remove_task_from_station(self, task: int, station: int) -> bool:
        ...
        try:
            if self.task_station_assignment[task] == station:
                ...
                LogManager.invalid_action(
                    "remove task from station, it wasnt assigned to it", self.name
                )
                return False
        except Exception as e:
    ...
    def can_task_be_assigned_to(
        ...
        reversed_orientation = self.reverse_default_graph_orientation
    ...
    def get_task_execution_time(
        ...
        task: int,
        worker: Optional[int] = None,
        custom_dict: Optional[Dict[int, List[int]]] = None,
    ) -> float:
        """
    ...
    def max_task_execution_time(
        ...
        task_times = (
            custom_dict[task]
            if custom_dict
            else self._bounded_task_execution_times[task]
        )
        if workers:
    ...
    def number_all_task_precedences(
        self, task: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        ...
        """
        result = (
            custom_dict or self.all_task_precedences[self.default_graph_orientation]
        )
        return len(result[task])

    def number_immediate_task_precedences(
        self, task: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        ...
        result = (
            custom_dict
            or self.immediate_task_precedences[self.default_graph_orientation]
        )
        return len(result[task])

    def number_all_task_followers(
        self, task: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        Returns the count of all (immediate and transitive) followers for a task.
        ...
        Returns:
            int: The count of all followers.
        """
        result = (
            custom_dict
            or self.all_task_precedences[self.reverse_default_graph_orientation]
        )
        return len(result[task])

    def number_immediate_task_followers(
        self, task: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        Returns the count of immediate followers for the given task.
        ...
        Returns:
            int: The count of immediate followers.
        """
        result = (
            custom_dict
            or self.immediate_task_precedences[self.reverse_default_graph_orientation]
        )
        return len(result[task])

    def decreasing_number_all_task_precedences(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
    ...
    def decreasing_number_immediate_task_precedences(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
    ...
    def decreasing_number_all_task_followers(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        Returns the negative count of all followers for a task.

        Useful for ordering tasks where fewer followers are preferred.
        """
        return -self.number_all_task_followers(task, custom_dict)

    def decreasing_number_immediate_task_followers(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> int:
        """
        Returns the negative count of immediate followers for a task.
        """
        return -self.number_immediate_task_followers(task, custom_dict)

    def decreasing_min_task_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
    ...
    def decreasing_average_task_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
    ...
    def __calculate_over_task_and_precedences(
        self,
        func: Callable[[int, Optional[int], Optional[Dict[int, List[int]]]], float],
        task: int,
        custom_dict: Optional[Dict[int, List[int]]],
    ) -> float:
        """
        ...
        return func(task, None, custom_dict) + sum(
            func(precedence, None, custom_dict) for precedence in precedences
        )

    def max_positional_weight_minus(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Computes the positional weight of a task as the sum of minimum execution times over the task and its precedences.
        """
        return self.__calculate_over_task_and_precedences(
            self.min_task_execution_time, task, custom_dict
        )

    def max_positional_weight_plus(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        ...
        """
        return self.__calculate_over_task_and_precedences(
            self.max_task_execution_time, task, custom_dict
        )

    def max_positional_weight_average(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        ...
        """
        return self.__calculate_over_task_and_precedences(
            self.average_task_execution_time, task, custom_dict
        )

    def difference_to_best_worker(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        ...
        return self.get_task_execution_time(
            task, worker, custom_dict
        ) - self.get_task_execution_time(task, best_worker, custom_dict)

    def ratio_to_best_worker(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        ...
        """
        best_worker = self.get_best_worker_for_task(task)
        return self.get_task_execution_time(task, worker, custom_dict) / (
            self.get_task_execution_time(task, best_worker, custom_dict) or 1
        )

    def ratio_of_number_all_task_precedences_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        ...
        """
        return self.number_all_task_precedences(task, custom_dict) / (
            self.get_task_execution_time(task, worker, custom_dict) or 1
        )

    def ratio_of_number_immediate_task_precedences_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
    ...
    def decreasing_ratio_of_number_all_task_precedences_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Returns the negative ratio of the total number of precedences to the task execution time.
        """
        return -self.ratio_of_number_all_task_precedences_over_time(
            task, worker, custom_dict
        )

    def decreasing_ratio_of_number_immediate_task_precedences_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Returns the negative ratio of the immediate precedences count to the task execution time.
        """
        return -self.ratio_of_number_immediate_task_precedences_over_time(
            task, worker, custom_dict
        )

    def ratio_of_number_all_task_followers_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Computes the ratio of the total number of followers for a task to its execution time.
        """
        return self.number_all_task_followers(task, custom_dict) / (
            self.get_task_execution_time(task, worker, custom_dict) or 1
        )

    def ratio_of_number_immediate_task_followers_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Computes the ratio of the immediate followers count for a task to its execution time.
        """
        return self.number_immediate_task_followers(task, custom_dict) / (
            self.get_task_execution_time(task, worker, custom_dict) or 1
        )

    def decreasing_ratio_of_number_all_task_followers_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Returns the negative ratio of the total number of followers to the task execution time.
        """
        return -self.ratio_of_number_all_task_followers_over_time(
            task, worker, custom_dict
        )

    def decreasing_ratio_of_number_immediate_task_followers_over_time(
        self, task: int, worker: int, custom_dict: Optional[Dict[int, List[int]]]
    ) -> float:
        """
        Returns the negative ratio of the immediate followers count to the task execution time.
        """
        return -self.ratio_of_number_immediate_task_followers_over_time(
            task, worker, custom_dict
        )
    ...
    def __get_func_for_task_ordering_rules(
        ...
        if task_ordering_rule == TaskOrderingRule.MAX_F:
            return self.decreasing_number_all_task_followers
        elif task_ordering_rule == TaskOrderingRule.MAX_IF:
            return self.decreasing_number_immediate_task_followers
            ...
            return self.decreasing_ratio_of_number_all_task_followers_over_time
    ...
    def _calculate_task_ordering_rules(
        ...
        if not priority_matrix:
            ...
            result_storage: Dict[TaskOrderingRule, Dict[int, Tuple[float, ...]]] = {  # type: ignore
                weight_type: {
                    task: (-1.0,) * self._number_of_workers for task in self.tasks
                }
                for weight_type in EnumUtil.get_values(TaskOrderingRule)
            }
        ...
        for task in self.tasks:
            for task_ordering_rule in EnumUtil.get_values(TaskOrderingRule):
                if isinstance(task_ordering_rule, TaskOrderingRule):
                    ...
                    task_rule_dict[task] = tuple(
                        map(
                            lambda worker: weight_function(
                                task, worker, priority_matrix
                            ),
                            self.workers,
                        )
                    )
    ...
    def get_worker_min_rlb(
        self, worker: int, override_unassigned_tasks: Iterable[int] = []
    ) -> int:
        """
