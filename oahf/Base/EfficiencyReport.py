...
class EfficiencyReport(Entity):
    ...
    def report_apply_improvement(
        self, new_eval: "Evaluation", old_eval: "Evaluation"
    ) -> None:
        """
        ...
        """
        improvement = (
            new_eval.get_objective_function() - old_eval.get_objective_function()
        )
    ...
    def report_apply_start(self) -> None:
        ...
        self.count_apply += 1
        self.events.append(
            (time.time() * 1000, Event(Event.TYPE.APPLY_START))
    ...
    def report_apply_end(self) -> None:
        ...
        self.total_time_for_apply += (
            time.time() - self.start_time_apply
    ...
    def report_unapply_end(self) -> None:
        ...
        self.total_time_for_unapply += (
            time.time() - self.start_time_unapply
        ) * 1000  # Time in milliseconds
    ...
    def __str__(self) -> str:
        ...
        avg_improvement = (
            (
                self.summed_improvement
                / (self.count_apply - self.count_unapply - self.count_apply_failed)
            )
            if (self.count_apply - self.count_unapply - self.count_apply_failed) > 0
            else 0
        )
        avg_search_time = (
            (self.total_time_for_search / self.count_searches)
            if self.count_searches > 0
            else 0
        )
        avg_apply_time = (
            (self.total_time_for_apply / self.count_apply)
            if self.count_apply > 0
            else 0
        )
        avg_unapply_time = (
            (self.total_time_for_unapply / self.count_unapply)
            if self.count_unapply > 0
            else 0
        )
        constraints_summary = "\n".join(
            [
                f"Constraint {key} Times: {value}"
                for key, value in sorted(
                    self.constraint_per_unapply.items(), key=lambda x: x[1]
                )
            ]
        )

        return (
            f"Report for {self.name}. Num. Searches: {self.count_searches}, Num. Apply: {self.count_apply}, "
