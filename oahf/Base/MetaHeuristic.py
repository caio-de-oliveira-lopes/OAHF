...
class MetaHeuristic(ABC):
    ...
    def __init__(self, 
        ...
        self.meta_heuristics_used: List['MetaHeuristic'] = meta_heuristics_used if isinstance(meta_heuristics_used, list) else [meta_heuristics_used] if meta_heuristics_used else []
    ...
    def get_efficiency_reports(self) -> Optional[List[Tuple[type, str]]]:
        if self.neighborhood_selection is None:
            ...
            return [(type(x), x.get_efficiency_report()) for x in self.neighborhood_selection.get_all()]
    ...
    def get_efficiency_reports_to_json(self) -> MetaHeuristicReport:
        ...
        if self.neighborhood_selection is None:
            report.reports = [x.get_efficiency_reports_to_json() for x in self.meta_heuristics_used]
        else:
            report.reports = [
    ...
    def run_operation(self, sol: 'Pool', parent: Optional['MetaHeuristic'] = None) -> 'Pool':
        try:
            ...
            self.logger.warning(f"Something went wrong while trying to run {self.__class__}")
    ...
    def log_best_solution(self, eval: 'Evaluation'):
        if self.log_solutions:
            self.solution_reports.best_solutions.append((self.stop_criteria.current_status(), eval.get_objective_function()))

    def log_current_solution(self, eval: 'Evaluation'):
        if self.log_solutions:
            self.solution_reports.current_solutions.append((self.stop_criteria.current_status(), eval.get_objective_function()))
    ...
    def stop_on_evaluations(self, evs: List['Evaluation']) -> bool:
        return self.stop_criteria.stop_on_evaluations(evs) or (self.parent_metaheuristic is not None and self.parent_metaheuristic.stop_on_evaluations(evs))
